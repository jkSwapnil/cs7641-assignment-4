"""
This module executed hyper-parameter experiments on the BitString MDP.
...
for gamma, eps in grid_search:
    ...
    value_iter = ValueIteration(bitstrings, gamma=gamma, epsilon=eps)
...
ax.legend()
fig.savefig("./plots/bitstrings_value_iterations.png")
plt.close(fig)
print("  - Results")
...
fig = plt.figure(figsize=(10,8))
ax = fig.add_subplot(1,1,1)
bit_string_value = list(range(len(optimum_policy)))
optimum_action = list(optimum_policy.values())
ax.bar(bit_string_value, optimum_action, color="blue")
ax.set_xlabel("Integer value of the Bit string")
ax.set_ylabel("Action according to optimum policy")
ax.set_title("Optimum action VS Int. value of Bit string")
fig.savefig("./plots/bitstrings_value_iterations_optimum_policy.png")
plt.close(fig)
print()
...
grid_search = [(0.1, 0.0001), (0.9, 0.0001), (0.9, 0.01)]  # List of hyper-param (gamma, eps)
...
for gamma, eps in grid_search:
    begin_tstamp = time.time()
    policy_iter = PolicyIteration(bitstrings, gamma=gamma, epsilon=eps)
...
ax1.legend()
fig1.savefig("./plots/bitstrings_policy_iterations_V.png")
plt.close(fig1)
ax2.set_xlabel("Number of iterations")
ax2.set_ylabel("Mean policy changes")
ax2.set_title("Mean policy changes VS No. of iterations for Policy Iterations")
ax2.legend()
fig2.savefig("./plots/bitstrings_policy_iterations_policy_changes.png")
plt.close(fig2)
print("  - Results")
print("\tGamma\tEpsilon\tExecution time\tV(s)")
...
ax.set_ylabel("Action according to optimum policy")
ax.set_title("Optimum action VS Int. value of Bit string")
fig.savefig("./plots/bitstrings_policy_iterations_optimum_policy.png")
plt.close(fig)
print()
...
for alpha, eps in grid_search:
    begin_tstamp = time.time()
    q_learning = QLearning(bitstrings, alpha=alpha, eps=eps)
    trained_policy, mean_state_values = q_learning(num_episodes=35000)
...
ax.legend()
fig.savefig("./plots/bitstrings_q_learning.png")
plt.close(fig)
print("  - Results")
...
ax.set_ylabel("Action according to optimum policy")
ax.set_title("Optimum action VS Int. value of Bit string")
fig.savefig("./plots/bitstrings_q_learning_optimum_policy.png")
plt.close(fig)
print()
